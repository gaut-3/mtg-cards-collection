...
function bookmarkletCode(appUrl: string) {
  return `javascript:(()=>{const u=location.href;if(!new RegExp('moxfield\\\\.com/decks/','i').test(u)){alert('Open a Moxfield deck page first.');return}const name=(document.querySelector('h1')?.innerText||document.title.replace(/\\s*-\\s*Moxfield.*$/,'')||'Moxfield deck').trim();const valid=t=>(t||'').split('\\n').filter(l=>/^\\s*\\d+x?\\s+/.test(l)).length;let chunks=[];document.querySelectorAll('textarea').forEach(e=>chunks.push(e.value||''));document.querySelectorAll('.cm-line,.cm-content [role="presentation"],[contenteditable="true"]').forEach(e=>chunks.push(e.innerText||e.textContent||''));let text=chunks.sort((a,b)=>valid(b)-valid(a))[0]||'';if(valid(text)<5)text=prompt('Could not find enough deck text automatically. In Moxfield edit view, copy the deck text and paste it here:')||'';if(!text.trim())return;const data=btoa(unescape(encodeURIComponent(JSON.stringify({name,url:u,text}))));location.href='${appUrl}/moxfield#moxfield-import='+data})()`
}
